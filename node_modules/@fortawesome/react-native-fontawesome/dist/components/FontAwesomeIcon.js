...
({
     ...
     secondaryOpacity= defaultProps.secondaryOpacity
 }) {
    var iconArgs = icon,
        maskArgs = mask,
        maskId = maskId,
    ...
    var style = _reactNative.StyleSheet.flatten(style);

    var iconLookup = normalizeIconArgs(iconArgs);
    var transform = objectWithKey('transform', typeof transform === 'string' ? _fontawesomeSvgCore.parse.transform(transform) : transform);
    ...
    var color = color || style.color || DEFAULT_COLOR; // This is the color that will be passed to the "fill" prop of the secondary Path element child (in Duotone Icons)
    // `null` value will result in using the primary color, at 40% opacity

    var secondaryColor = secondaryColor || color; // Secondary layer opacity should default to 0.4, unless a specific opacity value or a specific secondary color was given

    var secondaryOpacity = secondaryOpacity || DEFAULT_SECONDARY_OPACITY; // To avoid confusion down the line, we'll remove properties from the StyleSheet, like color, that are being overridden
